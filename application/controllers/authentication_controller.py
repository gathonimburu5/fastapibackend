...
def login(username: str = Form(), password: str = Form(), db: Session = Depends(get_db)):
    user_record = authService.authenticate_user(db, username, password)
    if not user_record:
    ...
    access_token = create_access_token({"sub": str(user_record.id), "username": user_record.username, "email": user_record.email_address })
    return {
