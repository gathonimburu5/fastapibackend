...
@customer_router.get("/customers", response_model=list[CustomerOut], dependencies=[Depends(get_current_user)])
def get_customers(db: Session = Depends(get_db)):
    return customerService.getAllCustomers(db)

@customer_router.post("/customers", response_model=CustomerOut, dependencies=[Depends(get_current_user)])
...
@customer_router.get("/customers/{customer_id}", response_model=CustomerOut, dependencies=[Depends(get_current_user)])
...
@customer_router.put("/customers/{customer_id}", response_model=CustomerOut, dependencies=[Depends(get_current_user)])
